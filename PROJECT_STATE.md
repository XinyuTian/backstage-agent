...
- The workbench keeps source-grounded evidence in a single-column upper detail area, omits empty sections, flattens extraction wrappers into plain requirements, and collapses original listing text; a spreadsheet-style component grid remains at the bottom with the pre-cap total as its first column, one agent-score row, and one correction row.
