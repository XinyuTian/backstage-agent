...
- Candidate-only score-review workbench in `src/backstage_agent/ui.py` at `http://127.0.0.1:8765/candidates`, with compact date navigation, exact-day and seven-day windows, a date-ordered candidate list, simplified extracted evidence, immutable agent scores, and independently saved component corrections.
...
- The workbench keeps evidence in the upper detail area and a spreadsheet-style component grid at the bottom with one agent-score row and one correction row; correction inputs save on Enter without interrupting multi-digit entry and retain the active date/window context.
- The visible `7 days` control toggles between exact-day and seven-day views while preserving the selected end date and query, and exposes its active state accessibly.
